...
namespace fr_public{
	...
	class BinaryMemory{
		public:
			BinaryMemory();
			BinaryMemory(const void *_buffer, int _size);
			BinaryMemory(const Byte *_buffer, int _size);
			BinaryMemory(const char *_buffer, int _size);
			BinaryMemory(const short *_buffer, int _size);
			BinaryMemory(const int *_buffer, int _size);
		...
		public:
			...
			void setMaxLimit(int _maxLimit){ m_MaxLimit = _maxLimit; }
			std::string print(std::string expand = "")const;

			inline void add_cur_size(size_t cur_size){ if(cur_size > 0 && m_CurBufferSize + cur_size <= m_MaxBufferSize){ m_CurBufferSize += cur_size; } }
			...
			inline int size()const{ return m_CurBufferSize; }
			inline int maxSize()const{ return m_MaxBufferSize; }
			inline int maxLimit()const{ return m_MaxLimit; }
		private:
