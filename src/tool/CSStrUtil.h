...
namespace Universal{
	...
	//! \brief	未完成。	
	//! \todo	
	void ConvertToUtf8( string &str, const eCSCharacterEncoding &originalEncoding );
	...
	void convertFullToHalf( string &str, eCSCharacterEncoding encoding = eEncoding_UTF8 );
	//! \biref	字符 半角转全角 的函数。
	//! \note	暂时仅支持UFT8，其他字符的支持需要扩展。
	//! \param[in,out] str 输出及输出。
	//! \param[in] encoding 编码参数，暂时仅支持UFT8
	void convertHalfToFull( string &str, eCSCharacterEncoding encoding = eEncoding_UTF8 );
	//! \brief	得到不同编码的字符长度。
	//! \note	暂时仅支持UFT8。
	//! \todo	下一步建议扩展GBK。
	size_t getStrSize( const string &str, eCSCharacterEncoding encoding = eEncoding_UTF8 );
} // namespace Universal{
